// Scriptura Bloomerang Tools — CORE LOGIC
//
// This file is NOT a userscript and is never installed directly. The
// installed script (bloomerang-tools.user.js) is a tiny loader that fetches
// this file fresh on every page load and runs it. That means every change
// made here takes effect for every browser, immediately, with nothing for
// staff to reinstall. Only the loader itself has a userscript "@version";
// this file doesn't need one for that mechanism, but CORE_VERSION below is
// still bumped on real changes, just as a quick way to confirm in the
// browser console which copy of the logic is actually running.
//
// See README.md, "Changing the logic", before editing this file.
...
(function () {
  ...
  // Keep the popup open while the mouse is over it (needed now that long
  // popups can be scrolled), and let it close the normal way on leaving it.
  ...
  // Scrolling or clicking inside the popup itself (its own scrollbar, or
  // selecting its text) must not count as "away".
  window.addEventListener('scroll', function (e) {
    if (popup.contains(e.target)) return;
    hidePopup();
  }, true);
  document.addEventListener('click', function (e) {
    if (activeIcon && !e.target.classList.contains('scriptura-help-icon') && !popup.contains(e.target)) {
      hidePopup();
    }
  });
